...
// Main content component that handles the profile page functionality
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {childContext?.selectedChild && childContext.selectedChild.id !== params.childId && (
                <button
                  onClick={() => {
                    const childToSelect = childContext.availableChildren.find(c => c.id === params.childId);
                    if (childToSelect) {
                      childContext.selectChild(childToSelect);
                    }
                  }}
                  className="px-3 py-1 bg-white/20 rounded text-white hover:bg-white/30 transition-colors"
                >
                  Switch to {childProfile.name}
                </button>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="flex items-start gap-6 mb-6">
            <AvatarUpload
              ...
              onUploadSuccess={() => {
                // TODO: Update avatar URL when API supports it
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setTheme(theme.id); // Use theme.id since setTheme expects a string
...
function WrappedChildProfilePage({ params }: { params: { childId: string } }) {
  const subdomainInfo = getSubdomainInfo();
  ...
  if (!familySlug) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600">No family context found.</p>
        </div>
      </div>
    );
  }
...
export default WrappedChildProfilePage;